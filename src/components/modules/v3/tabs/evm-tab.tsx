...
import { ProjectData, DomainAggregatesMonth, AirStation, NoiseStation, SoilStation } from '../v3-types'
...
export function EvmTab({ projects, month, domainAggregates, onSelectProject }: EvmTabProps) {
  ...
  const statutory = evmAgg?.statutory || {
    'Environmental Clearance (EC)': { yes: 41, no: 1, na: 3, applied: 1, blank: 0 },
    'Consent to Establish (CTE) - Plants': { yes: 40, no: 2, na: 2, applied: 2, blank: 0 },
    'Consent to Operate (CTO) - Plants': { yes: 39, no: 3, na: 2, applied: 2, blank: 0 },
    'Groundwater SGWB Extraction Permission': { yes: 44, no: 0, na: 1, applied: 1, blank: 0 },
    'PESO Petroleum / Fuel Storage License': { yes: 38, no: 2, na: 4, applied: 2, blank: 0 },
    'APWALTA Tree Transit / Cutting Permission': { yes: 42, no: 1, na: 2, applied: 1, blank: 0 },
    'Hazardous Waste Authorization (HWA)': { yes: 40, no: 2, na: 2, applied: 2, blank: 0 }
  }
  ...
  const soilStations: SoilStation[] = evmAgg?.soil_stations || [
    { code: 'S1', name: 'Rayapudi Soil Point 1', gps: '16.512, 80.524', lat: 16.512, lng: 80.524, status: 'Normal' },
    { code: 'S2', name: 'Sakhamuru Reservoir Soil', gps: '16.535, 80.548', lat: 16.535, lng: 80.548, status: 'Normal' },
    { code: 'S3', name: 'Amaravati Core Soil S3', gps: '16.558, 80.572', lat: 16.558, lng: 80.572, status: 'Normal' }
  ]

  // Visual D: Water & Wastewater
  const waterWW = evmAgg?.water_wastewater || {
    consumption: { construction: 1420, domestic: 680, dust_suppression: 850, total: 2950 },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <CardHeader className="p-4 border-b bg-muted/20 flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Evaluation across all projects | "Applied" is never counted as compliant
              </p>
            </div>
            ...
            <div className="max-h-[340px] overflow-y-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-muted/60 sticky top-0 z-10 border-b">
                  <tr>
                    <th className="p-2.5 font-semibold text-foreground">Statutory Approval / Act</th>
                    <th className="p-2.5 font-semibold text-center text-foreground w-36">Response Ratio</th>
                    <th className="p-2.5 font-semibold text-center text-foreground">Yes</th>
                    <th className="p-2.5 font-semibold text-center text-foreground">No</th>
                    <th className="p-2.5 font-semibold text-center text-foreground">Applied</th>
                    <th className="p-2.5 font-semibold text-center text-foreground">NA</th>
                    <th className="p-2.5 font-semibold text-center text-foreground">Blank</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {Object.entries(statutory).map(([name, sdata], idx) => {
                    const total = sdata.yes + sdata.no + sdata.applied + sdata.na + sdata.blank || 46
                      ...
                      <tr key={idx} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-2.5">
                          <div className="w-full h-3 rounded-full bg-muted/60 flex overflow-hidden">
                            <div style={{ width: `${yW}%` }} className="bg-emerald-500 h-full" title={`Yes: ${sdata.yes}`} />
                            <div style={{ width: `${nW}%` }} className="bg-rose-500 h-full" title={`No: ${sdata.no}`} />
                            <div style={{ width: `${aW}%` }} className="bg-amber-500 h-full" title={`Applied: ${sdata.applied}`} />
                            <div style={{ width: `${naW}%` }} className="bg-slate-300 h-full" title={`NA: ${sdata.na}`} />
                          </div>
                        </td>
                        <td className="p-2.5 text-center font-bold text-emerald-600 font-mono">{sdata.yes}</td>
                        <td className="p-2.5 text-center font-bold text-rose-600 font-mono">{sdata.no}</td>
                        <td className="p-2.5 text-center font-medium text-amber-600 font-mono">{sdata.applied}</td>
                        <td className="p-2.5 text-center text-muted-foreground font-mono">{sdata.na}</td>
                        <td className="p-2.5 text-center text-muted-foreground font-mono">{sdata.blank}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <Card className="border shadow-xs flex flex-col justify-between">
          <CardHeader className="p-4 border-b bg-muted/20">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Reuse % = (Reused / Generated) * 100 ({waterWW.wastewater.reuse_pct}%)
            </p>
          </CardHeader>
          <CardContent className="p-4 flex flex-col gap-4">
            {/* Water Consumption Breakdown */}
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold text-foreground">Water Consumption ({waterWW.consumption.total} KLD)</span>
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="p-2 border rounded-md bg-card">
                  <span className="text-[10px] text-muted-foreground">Construction</span>
                  <div className="font-bold text-foreground mt-0.5">{waterWW.consumption.construction} KLD</div>
                </div>
                <div className="p-2 border rounded-md bg-card">
                  <span className="text-[10px] text-muted-foreground">Domestic</span>
                  <div className="font-bold text-foreground mt-0.5">{waterWW.consumption.domestic} KLD</div>
                </div>
                <div className="p-2 border rounded-md bg-card">
                  <span className="text-[10px] text-muted-foreground">Dust Suppr.</span>
                  <div className="font-bold text-foreground mt-0.5">{waterWW.consumption.dust_suppression} KLD</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 border-t pt-3">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-foreground">Wastewater Reuse Ratio</span>
                <Badge className="bg-blue-500/15 text-blue-700 dark:text-blue-400 font-mono">
                  {waterWW.wastewater.reuse_pct}% Reused
                </Badge>
              </div>
              <Progress value={waterWW.wastewater.reuse_pct} className="h-2.5 [&>div]:bg-blue-500" />
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                <span>Generated: <strong>{waterWW.wastewater.generated} KLD</strong></span>
                <span>Treated: <strong>{waterWW.wastewater.treated} KLD</strong></span>
                <span>Reused: <strong>{waterWW.wastewater.reused} KLD</strong></span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader className="p-4 border-b bg-muted/20 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-sm font-bold text-foreground flex items-center gap-2">
              ...
              SVG Geographic Map: Environmental Monitoring Network (Amaravati Capital Region)
            </CardTitle>
            <p className="text-xs text-muted-foreground mt-0.5">
              Interactive plots for Air (AAQ), Noise (N), and Soil (S) stations. Click any point to inspect exact parameters.
            </p>
          </div>
          ...
          <div className="flex-1 bg-slate-950/5 dark:bg-slate-900/40 rounded-xl border border-border/80 relative min-h-[360px] p-4 flex items-center justify-center overflow-hidden">
            ...
            <svg className="w-full h-[340px]" viewBox="0 0 800 400">
              ...
              <path
                ...
                strokeOpacity="0.35"
              />
              <text x="70" y="100" fill="#0284c7" fontSize="12" fontWeight="bold">Krishna River</text>

              {/* Capital City Zone Boundary Guides */}
              <rect x="100" y="140" width="580" height="230" rx="15" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" className="text-muted-foreground/30" />
              <text x="120" y="165" fill="currentColor" fontSize="11" className="text-muted-foreground/50">Amaravati Core Capital Region</text>
              ...
              {airStations.map((stn, i) => {
                ...
                return (
                  <g
                    key={`air-${i}`}
                    onClick={() => setSelectedStation({ ...stn, type: 'Air Quality' })}
                  ...
                  >
                    <circle
                      ...
                      cy={y}
                      r={isSelected ? 10 : 7}
                      className={stn.exceedance ? 'fill-rose-500 animate-pulse' : 'fill-emerald-500'}
                      stroke="#ffffff"
                      strokeWidth="2"
                    />
                    <text x={x + 9} y={y + 4} fill="currentColor" fontSize="10" fontWeight="bold" className="text-foreground select-none">
                      {stn.code}
                    </text>
                  </g>
                )
              ...
              {noiseStations.map((nstn, i) => {
                ...
                return (
                  <g
                    key={`noise-${i}`}
                    onClick={() => setSelectedStation({ ...nstn, type: 'Noise Monitoring' })}
                    className="cursor-pointer transition-transform hover:scale-125"
                  >
                    <rect
                      x={x - 6}
                      ...
                      strokeWidth="2"
                    />
                    <text x={x + 8} y={y + 4} fill="currentColor" fontSize="10" fontWeight="bold" className="text-foreground select-none">
                      {nstn.code}
                    </text>
                  </g>
                )
              ...
              {soilStations.map((sstn, i) => {
                ...
                return (
                  <g
                    key={`soil-${i}`}
                    onClick={() => setSelectedStation({ ...sstn, type: 'Soil Sampling' })}
                    className="cursor-pointer transition-transform hover:scale-125"
                  >
                    <polygon
            ...
            {selectedStation ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-start justify-between">
                  <div>
                    <Badge className="mb-1 text-[10px]">{selectedStation.type}</Badge>
                    <h4 className="text-sm font-bold text-foreground">{selectedStation.name}</h4>
                    <span className="text-[11px] font-mono text-muted-foreground block">{selectedStation.code} • {selectedStation.gps || 'GPS Tagged'}</span>
                  </div>
                ...
                {selectedStation.type === 'Air Quality' && (
                  <div className="flex flex-col gap-2 pt-2 border-t text-xs">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">PM10 (Limit &le; 100):</span>
                      <span className={`font-mono font-bold ${selectedStation.pm10 > 100 ? 'text-rose-600' : 'text-foreground'}`}>
                        {selectedStation.pm10 ?? '-'} µg/m³ {selectedStation.pm10 > 100 ? '▲ (Exceeded)' : ''}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">PM2.5 (Limit &le; 60):</span>
                      <span className={`font-mono font-bold ${selectedStation.pm25 > 60 ? 'text-rose-600' : 'text-foreground'}`}>
                        {selectedStation.pm25 ?? '-'} µg/m³ {selectedStation.pm25 > 60 ? '▲ (Exceeded)' : ''}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">SO2 (Limit &le; 80):</span>
                      <span className="font-mono">{selectedStation.so2 ?? '-'} µg/m³</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">NOx (Limit &le; 80):</span>
                      <span className="font-mono">{selectedStation.nox ?? '-'} µg/m³</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">CO (Limit &le; 2.0):</span>
                      <span className="font-mono">{selectedStation.co ?? '-'} mg/m³</span>
                    </div>
                    <div className="p-2 rounded-md bg-muted/40 mt-1">
                      <span className="text-[10px] text-muted-foreground">Project: <strong>{selectedStation.projectName}</strong></span>
                    </div>
                  </div>
                )}
                ...
                {selectedStation.type === 'Noise Monitoring' && (
                  <div className="flex flex-col gap-2 pt-2 border-t text-xs">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Day Leq (Limit &le; 75):</span>
                      <span className="font-mono font-bold">{selectedStation.lday ?? selectedStation.leq ?? '-'} dB(A)</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Night Leq (Limit &le; 70):</span>
                      <span className="font-mono font-bold">{selectedStation.lnight ?? '-'} dB(A)</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Lmax:</span>
                      <span className="font-mono">{selectedStation.lmax ?? '-'} dB(A)</span>
                    </div>
                  </div>
                )}
                ...
                {selectedStation.type === 'Soil Sampling' && (
                  <div className="flex flex-col gap-2 pt-2 border-t text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-emerald-600">Compliant (Baseline)</span>
                    </div>
                    <span className="text-[11px] text-muted-foreground">Heavy metals & pH within prescribed agricultural standards</span>
                  </div>
                )}
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card className="border shadow-xs">
          <CardHeader className="p-4 border-b bg-muted/20">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Standards: PM10 &gt; 100, PM2.5 &gt; 60, SO2 &gt; 80, NOx &gt; 80, CO &gt; 2.0 | Alert: Red, Bold, and " ▲"
            </p>
          </CardHeader>
          <CardContent className="p-0">
        ...
        <Card className="border shadow-xs">
          ...
          <CardContent className="p-4">
            <div className="max-h-[320px] overflow-y-auto flex flex-col gap-3 pr-2">
              {wasteList.map((w, idx) => {
                const dispPct = Math.min(100, Math.round((w.disposed / Math.max(1, w.generated)) * 100))
                return (
                  <div key={idx} className="flex flex-col gap-1 p-2 rounded-md hover:bg-muted/30 transition-colors">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono text-[11px] text-muted-foreground">
                        {w.disposed.toLocaleString()} / {w.generated.toLocaleString()} {w.unit} ({dispPct}%)
                      </span>
                    </div>
                    <Progress value={dispPct} className="h-2 [&>div]:bg-emerald-500" />
                  </div>
                )
              })}
            </div>
