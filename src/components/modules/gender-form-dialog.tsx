...
export default function GenderFormDialog({ open, onOpenChange, onSaved }: { open: boolean; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: any) => {
      const res = await fetch('/api/es-forms/Gender', {
        method: 'POST',
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['es-forms', 'Gender'] })
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div>
                    ...
                    <div className="mb-6 space-y-2">
                      ...
                      <Select value={data.gbvInstances} onValueChange={v => setData({...data, gbvInstances: v as any, gbvDetails: v === 'Yes' ? (data.gbvDetails.length > 0 ? data.gbvDetails : [emptyGbvDetail()]) : []})}>
